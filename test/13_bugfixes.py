...
class DummyResponder(BaseResponder):
    def respond(self, user_input: str) -> AgentResponse:
        return AgentResponse(response="test", summary="test", requires_user_feedback=False)
...
# Non-greedy should pick the first one
assert parsed.response == "first", f"Expected 'first', got {parsed.response!r}"
...
responder_with_dash = DummyResponder(spych_object=spych_object, dashboard=dashboard, use_speaker=False)

# Manually simulate a turn that returns an empty response
dashboard.on_user_input("hello")
assert dashboard._current_user_input == "hello"

# Simulate final empty response
empty_resp = AgentResponse(response="", summary="", requires_user_feedback=False)
# In responders.py, we added:
...
assert dashboard._current_user_input == "", "Dashboard should have cleared user input"

# 3. Verify scroll boundary
dashboard.scroll_up(10000)
assert dashboard._scroll_offset == 5000, f"Expected 5000, got {dashboard._scroll_offset}"
dashboard.scroll_down(6000)
assert dashboard._scroll_offset == 0, f"Expected 0, got {dashboard._scroll_offset}"

print("All bugfix assertions passed.")
